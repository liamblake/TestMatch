...
std::string Innings::print() {
  ...
  // Header
  output +=
      DIVIDER + BUFFER + team_bat->name + " " + ordinal(inns_no) + " Innings\n";
  ...
  for (int i = 0; i < 11; i++) {
    ...
    if (ptr->is_active()) {
      output += BUFFER + ptr->get_player_ptr()->get_full_initials() + BUFFER +
                ptr->print_dism() + BUFFER + std::to_string(stats.runs) +
                BUFFER + std::to_string(stats.balls) + BUFFER +
  ...
  // Bowlers
  output += BUFFER + "Bowling" + BUFFER + BUFFER + "O" + BUFFER + "M" + BUFFER +
            "R" + BUFFER + "W" + BUFFER + "Econ" + DIVIDER;
  for (int i = 0; i < 11; i++) {
  ...
  while (inns_i < 4) {
    ...
    if (inns_i == 2 && inns_state == "allout" && lead < 0) {
      ...
      if (inns_state == "allout") {
        ...
        // Raise an exception, Innings::simulate() has returned something
        // unknown
